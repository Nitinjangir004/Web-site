...
export default function ComicDetailPage() {
  ...
  const pdfAvailable = comic.slug === 'the-hope';
  const pdfPath = pdfAvailable ? `/Comics/TheHopeHarsh.pdf` : '';
            ...
            {allComics.filter(item => item.id !== comic.id).slice(0, 4).map(item => (
              <div key={item.id} className="flex flex-col items-center bg-white rounded-2xl shadow-lg p-4">
                ...
                <div className="w-full">
                  ...
                  <Link href={`/toons-and-tells/comic/${item.slug}`} className="btn-secondary w-full text-center block">
                    Read Comic
                  </Link>
                </div>
              </div>
